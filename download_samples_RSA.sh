data='samples_names_RSA.txt'
#skip the header row. 
tail +2 wastewater_ncbi_RSA.csv | cut -d, -f1 > $data
...
if [ ! -f "vars/${sample}.tsv" ]; then
	...
	ivar trim -x 4 -e -m 80 -i "bams/${sample}.sorted.bam" -b bedfiles/nCoV-2019_v1.bed -p "trimmed/${sample}.trimmed"
