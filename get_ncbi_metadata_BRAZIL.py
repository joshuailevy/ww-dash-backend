...
def pull_sample_bed(accID):
    df = pd.read_csv('wastewater_ncbi_BRAZIL.csv',index_col=0)
...
if __name__=="__main__":
    ...
    handle = Entrez.esearch(db="sra", idtype='acc', retmax=500,
                            sort='recently_added',
                            term="SARS-CoV-2[All Fields] AND (wastewater[All Fields] AND surveillance[All Fields] AND brazil[All Fields])") 
    ...
    df.to_csv('wastewater_ncbi_BRAZIL.csv')
