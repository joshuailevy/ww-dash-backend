data='samples_names_SWISS.txt'
#skip the header row. 
tail +2 wastewater_ncbi_SWISS.csv | cut -d, -f1 > $data
...
if [ ! -f "vars/${sample}.tsv" ]; then
	...
	# ivar trim -x 20 -e -m 80 -i "bams/${sample}.sorted.bam" -p "trimmed/${sample}.trimmed"
	...
	freyja variants "bams/${sample}.sorted.bam" --variants "vars/${sample}.tsv" --depths "depths/${sample}.depth" --ref data/NC_045512_Hu-1.fasta --annot data/NC_045512_Hu-1.gff
