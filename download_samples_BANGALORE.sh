data='samples_names_bangalore.txt'
#skip the header row. 
tail +2 wastewater_ncbi_bangalore.csv | cut -d, -f1 > $data
...
# if [ ! -f "vars/${sample}.tsv" ]; then
	...
	minimap2 -ax sr data/NC_045512_Hu-1.fasta ${sample}.fastq  | samtools view -bS - > "bams/${sample}.bam"
	rm ${sample}.fastq 

	samtools sort -o "bams/${sample}.sorted.bam" "bams/${sample}.bam"
	samtools index "bams/${sample}.sorted.bam" 
	ivar trim -x 4 -e -m 80 -i "bams/${sample}.sorted.bam" -b bedfiles/ARTICv3.bed -p "trimmed/${sample}.trimmed"
